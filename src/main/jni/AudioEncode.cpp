 ...
 * Constants
 **/
static char const * const AudioEncode_classname   = "com.entscheidungsbaum.nativesound.jni.AudioEncode";
static char const * const AudioEncode_mObject     = "mObject";

static char const * const IllegalArgumentException_classname  = "java.lang.IllegalArgumentException";

static char const * const LTAG                          = "AudioEncode/native";
...
/*****************************************************************************
 * Native AudioEncode representation
 *
 * AudioEncode uses a writer thread to write its internal buffer. The
...
public:
  ...
  struct trampoline
  {
    typedef void * (AudioEncode::* func_t)(void * args);

    AudioEncode * m_encoder;
    func_t              m_func;
    void *              m_args;

    trampoline(AudioEncode * encoder, func_t func, void * args)
      : m_encoder(encoder)
      , m_func(func)
      , m_args(args)
    {
   ...
   **/
  AudioEncode(char * outfile, int sample_rate, int channels,
      int bits_per_sample)
    : m_outfile(outfile)
  ...
  char const * const init()
  {
    ...
    // Start thread!
    err = pthread_create(&m_writer, NULL, &AudioEncode::trampoline_func,
        new trampoline(this, &AudioEncode::writer_thread, NULL));
    if (err) {
...
private:
  ...
  static void * trampoline_func(void * args)
  {
    ...
    AudioEncode * encoder = tramp->m_encoder;
...
/**
 * Retrieve AudioEncode instance from the passed jobject.
 **/
static AudioEncode * get_encoder(JNIEnv * env, jobject obj)
{
  assert(sizeof(jlong) >= sizeof(AudioEncode *));

  // Do the JNI dance for getting the mObject field
  jclass cls = env->FindClass(AudioEncode_classname);
  jfieldID object_field = env->GetFieldID(cls, AudioEncode_mObject, "J");
  jlong encoder_value = env->GetLongField(obj, object_field);

  env->DeleteLocalRef(cls);

  return reinterpret_cast<AudioEncode *>(encoder_value);
}


/**
 * Store AudioEncode instance in the passed jobject.
 **/
static void set_encoder(JNIEnv * env, jobject obj, AudioEncode * encoder)
{
  assert(sizeof(jlong) >= sizeof(AudioEncode *));
...
jint
Java_fm_nativesound_jni_AudioEncode_write(JNIEnv * env, jobject obj,
...
void
Java_fm_nativesound_jni_AudioEncode_flush(JNIEnv * env, jobject obj)
{
  AudioEncode * encoder = get_encoder(env, obj);
...
jfloat
Java_fm_nativesound_jni_AudioEncode_getMaxAmplitude(JNIEnv * env, jobject obj)
{
  AudioEncode * encoder = get_encoder(env, obj);
...
jfloat
Java_fm_nativesound_jni_AudioEncode_getAverageAmplitude(JNIEnv * env, jobject obj)
{
  AudioEncode * encoder = get_encoder(env, obj);

 ...
 * Constants
 **/
static char const * const AudioDecode_classname   = "com.entscheidungsbaum.nativesound.jni.AudioDecode";
static char const * const AudioDecode_mObject     = "mObject";

static char const * const IllegalArgumentException_classname  = "java.lang.IllegalArgumentException";

static char const * const LTAG                          = "AudioDecoder/native";
...
/*****************************************************************************
 * Native AudioDecode representation
 **/
...
FLAC__StreamDecoderReadStatus flac_read_helper(
    ...
    void * client_data)
{
  AudioDecode * dc = static_cast<AudioDecode *>(client_data);
...
/**
 * Retrieve AudioDecode instance from the passed jobject.
 **/
static AudioDecode * get_decoder(JNIEnv * env, jobject obj)
{
  assert(sizeof(jlong) >= sizeof(AudioDecode *));

  // Do the JNI dance for getting the mObject field
  jclass cls = env->FindClass(AudioDecode_classname);
  jfieldID object_field = env->GetFieldID(cls, AudioDecode_mObject, "J");
  jlong decoder_value = env->GetLongField(obj, object_field);

  env->DeleteLocalRef(cls);

  return reinterpret_cast<AudioDecode *>(decoder_value);
}


/**
 * Store AudioDecode instance in the passed jobject.
 **/
static void set_decoder(JNIEnv * env, jobject obj, AudioDecode * decoder)
{
  assert(sizeof(jlong) >= sizeof(AudioDecode *));
...
void
Java_fm_nativesound_jni_AudioDecode_init(JNIEnv * env, jobject obj,
    jstring infile)
{
  assert(sizeof(jlong) >= sizeof(AudioDecode *));

	aj::log(ANDROID_LOG_DEBUG, LTAG,"AudioDecode_init.., infile=%s", aj::convert_jstring_path(env, infile));

  AudioDecode * decoder = new AudioDecode(
...
void
Java_fm_nativesound_jni_AudioDecode_deinit(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
  delete decoder;
...
jint
Java_fm_nativesound_jni_AudioDecode_read(JNIEnv * env, jobject obj,
...
jint
Java_fm_nativesound_jni_AudioDecode_bitsPerSample(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
jint
Java_fm_nativesound_jni_AudioDecode_channels(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
jint
Java_fm_nativesound_jni_AudioDecode_sampleRate(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
jint
Java_fm_nativesound_jni_AudioDecode_minBufferSize(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
jint
Java_fm_nativesound_jni_AudioDecode_totalSamples(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
void
Java_fm_nativesound_jni_AudioDecode_seekTo(JNIEnv * env, jobject obj, jint sample)
{
  AudioDecode * decoder = get_decoder(env, obj);
...
jint
Java_fm_nativesound_jni_AudioDecode_position(JNIEnv * env, jobject obj)
{
  AudioDecode * decoder = get_decoder(env, obj);
